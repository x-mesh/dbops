...
async fn fetch_members(ctx: &Ctx) -> Result<Vec<MemberInfo>> {
    ...
    let status_doc = match mongo_client.database("admin").run_command(doc! { "replSetGetStatus": 1 }).await {
    ...
    let rows = members
        ...
        .map(|m| {
            vec![
                m.name.clone(),
                m.state.clone(),
                if m.health >= 1.0 { "healthy".to_string() } else { "unhealthy".to_string() },
                m.lag_secs.map_or_else(|| "-".to_string(), |secs| secs.to_string()),
                m.last_heartbeat.map_or_else(
                    || "-".to_string(),
                    |dt| dt.try_to_rfc3339_string().unwrap_or_else(|_| "-".to_string()),
                ),
            ]
        })
        .collect();

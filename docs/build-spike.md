...
Owner: t2. This is the only task that adds dependencies to `Cargo.toml`.
Later tasks (t3+) build on top of the stack fixed here without adding new
crates.
...
- `cargo test` (dev/test profile, unaffected by `[profile.release]`): 264
  passed, 0 failed. This confirms `panic = "abort"` in the release profile
